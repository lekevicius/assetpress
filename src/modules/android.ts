...
import { resizeFilter, densities, allowedExtensions } from './android-constants'

import * as util from '../utilities'

const defaults: IAndroidOptions = {
  ldpi: false,
  xxxhdpi: false
}
...
export default class AndroidProcessor {
  
  constructor(
    private passedInputDirectory: string, 
    private passedOutputDirectory = '',
    ...
    let outputDirectoryName = passedOutputDirectory ? util.removeTrailingSlash(passedOutputDirectory) : 'res'
    let outputDirectoryBase = util.resolvePath(inputDirectory, '..')
    let outputDirectory = util.addTrailingSlash(util.resolvePath(outputDirectoryBase, outputDirectoryName))
    let temporaryDirectoryObject = new tmp.Dir
    let temporaryDirectory = util.addTrailingSlash(temporaryDirectoryObject.path)
    let temporaryPartsDirectoryObject = new tmp.Dir
    let temporaryPartsDirectory = util.addTrailingSlash(temporaryPartsDirectoryObject.path)

    let produceDensities = _.keys(densities)
    if (!options.ldpi) {
